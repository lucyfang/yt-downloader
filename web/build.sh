...
# ── 4. Version check — skip compile if already up to date ────────────────────
SOURCE_VERSION=$(grep -m1 'let appVersion' "$TMP/YTDownloader.swift" | grep -o '"[^"]*"' | tr -d '"')
INSTALLED_VERSION=$(/usr/libexec/PlistBuddy -c "Print CFBundleVersion" \
  /Applications/YTDownloader.app/Contents/Info.plist 2>/dev/null || echo "")

if [ "$SOURCE_VERSION" = "$INSTALLED_VERSION" ] && [ -x /Applications/YTDownloader.app/Contents/MacOS/YTDownloader ]; then
  echo "✓ YT Downloader $SOURCE_VERSION already installed — updating yt-dlp..."
  /Applications/YTDownloader.app/Contents/Resources/bin/yt-dlp -U
  echo ""
  echo "✅ Up to date! Opening YT Downloader..."
  echo ""
  open /Applications/YTDownloader.app
  exit 0
fi

echo "↓ Installing YT Downloader $SOURCE_VERSION..."
...
<plist version="1.0"><dict>
  ...
  <key>CFBundleVersion</key><string>${SOURCE_VERSION}</string>
